...
int console_execute(const char *str)
{
  ...
  command_length = strlen(u);    
    ...
    if (strcmp(u,"find")==0){
				char *type = 0, *dir = 0, *exe = 0, *find = 0;
				int typ = 0, checker = 0, ex = 0;

                dir = strtok(0," ");
				type = strtok(0, " ");
				exe = strtok(0, " ");

				//if file, typ = 1. if directory, typ = 2. else if no -f or -d, typ = 0
				if(strcmp(type,"-f")==0){
					typ = 1;					
				...
				//printf("dir: %s type: %s exe: %s \n", dir, type, exe);

				//if typ is 0, adjust by saving exe to find and saving type to exe
				//executable then ex = 1, name then ex = 2, else print error
				if(typ == 0){
					strcpy(find, exe);
					strcpy(exe, type);
					if(strcmp(exe,"-executable")==0){
						ex = 1;					
					}else if(strcmp(exe,"-name")==0){
						ex = 2;
					}else{
						printf("Invalid syntax. Please provide an option: -executable or -name\n");
					}
				}else{
					if(strcmp(exe,"-executable")==0){
						ex = 1;					
				...
				if (dir!=0){
					...
					if(strcmp(dir,".")==0){
						u=strtok(0,"\n");
						//if u has no value and typ = 0
						if(typ==0 && u==0){
							//save find to u and update checker
							strcpy(u, find);
							checker = 1;
						}	
						if(u!=0 || checker==1){
					...
					// / means root
						...
						char temp[255];
						//save current path to temp
               			strcpy(temp,showpath(temp));
						u=strtok(0,"\n");	
						//if u has no value and typ = 0
						if(typ==0 && u==0){
							//save find to u and update checker
					...
					//else if other directory	
						...
						strcpy(tofind, dir);
						if(dir!=0){	
							//pass dir to findDir. returns 1 if dir is found in current directory
							checker = findDir(dir);
							//if found
							if(checker == 1){
								char temp[255];
