...
void findfile(char *name, int type, int exe)
{
	char temp[255];
	int sizeOfPtr = 0;
    vfs_node *ptr=vfs_searchname(name);
	
	//printf("name: %s type: %d exe: %d\n", name, type, exe);


    if (ptr!=0 && type==1 && exe==2)	//if file and not executable
    {
		if(ptr->attb&FILE_DIRECTORY){	
			
		}else{
			printf("%s%s\n",showpath(temp),ptr->name);
		}
	}else if (ptr!=0 && type==2 && exe==2) //if directory and not executable
    {
        if(ptr->attb&FILE_DIRECTORY){	
			printf("%s%s\n",showpath(temp),ptr->name);
		}  
    }else if (ptr!=0 && (type==1 || type ==0) && exe==1)	//if file and executable
    {
		if(ptr->attb&FILE_DIRECTORY){	
			
		}else{
			//check if ptr->name ends in .exe
			sizeOfPtr = strlen(ptr->name);
			//printf("%c %c %c\n", ptr->name[sizeOfPtr-1], ptr->name[sizeOfPtr-2], ptr->name[sizeOfPtr-3]);
			if(ptr->name[sizeOfPtr-1] == 'e' && ptr->name[sizeOfPtr-2] == 'x' && ptr->name[sizeOfPtr-3] == 'e' && ptr->name[sizeOfPtr-4] == '.'){
				printf("%s%s\n",showpath(temp),ptr->name);
			}
		}
	}else if(ptr!=0 && type==0 && exe==2)	//if file/dir and not executable
	{
		printf("%s%s\n",showpath(temp),ptr->name);
	}
};
